...
contract('BPool', async (accounts) => {
    ...
    const govWallet = accounts[3];
    const colWallet = accounts[4];
    ...
    describe('Finalizing pool', () => {
        ...
        it('Fails setting low liquidity providers fee', async () => {
            await truffleAssert.reverts(
                pool.setLiquidityProvidersFee(toWei('0.0000001')),
                'ERR_MIN_FEE',
            );
        });

        it('Fails setting high liquidity providers fee', async () => {
            await truffleAssert.reverts(
                pool.setLiquidityProvidersFee(toWei('0.11')),
                'ERR_MAX_FEE',
            );
        });

        it('Fails setting high governance fee', async () => {
            await truffleAssert.reverts(
                pool.setGovernanceFee(toWei('0.11')),
                'ERR_MAX_FEE',
            );
        });

        it('Fails setting high collateralization fee', async () => {
            await truffleAssert.reverts(
                pool.setCollateralizationFee(toWei('0.11')),
                'ERR_MAX_FEE',
            );
        });

        it('Fails setting high swaps fees', async () => {
            await pool.setLiquidityProvidersFee(toWei('0.04'));
            await pool.setGovernanceFee(toWei('0.04'));
            await truffleAssert.reverts(
                pool.setCollateralizationFee(toWei('0.04')),
                'ERR_MAX_FEE',
            );
        });
        ...
        it('Fails nonadmin sets fees or controller', async () => {
            await truffleAssert.reverts(
                pool.setLiquidityProvidersFee(toWei('0.003'), { from: user1 }),
                'ERR_NOT_CONTROLLER',
            );
            await truffleAssert.reverts(
                pool.setGovernanceFee(toWei('0.003'), { from: user1 }),
                'ERR_NOT_CONTROLLER',
            );
            await truffleAssert.reverts(
                pool.setCollateralizationFee(toWei('0.003'), { from: user1 }),
                'ERR_NOT_CONTROLLER',
            );
            await truffleAssert.reverts(
        ...
        it('Fails nonadmin sets governance or collateralization wallets', async () => {
            await truffleAssert.reverts(
                pool.setGovernanceWallet(govWallet, { from: user1 }),
                'ERR_NOT_CONTROLLER',
            );
            await truffleAssert.reverts(
                pool.setCollateralizationWallet(colWallet, { from: user1 }),
                'ERR_NOT_CONTROLLER',
            );
        });

        it('Admin sets liquidity providers fee', async () => {
            await pool.setLiquidityProvidersFee(toWei('0.003'));
            const fee = await pool.getLiquidityProvidersFee();
            assert.equal(0.003, fromWei(fee));
        });

        it('Admin sets governance fee', async () => {
            await pool.setGovernanceFee(toWei('0.003'));
            const fee = await pool.getGovernanceFee();
            assert.equal(0.003, fromWei(fee));
        });

        it('Admin sets collateralization fee', async () => {
            await pool.setCollateralizationFee(toWei('0.003'));
            const fee = await pool.getCollateralizationFee();
            assert.equal(0.003, fromWei(fee));
        });

        it('Admin sets swaps fees', async () => {
            const swapFeeBefore = await pool.getSwapFee();
            assert.equal(0.009, fromWei(swapFeeBefore));
            await pool.setLiquidityProvidersFee(toWei('0.0015'));
            await pool.setGovernanceFee(toWei('0.0009'));
            await pool.setCollateralizationFee(toWei('0.0006'));
            const swapFeeAfter = await pool.getSwapFee();
            assert.equal(0.003, fromWei(swapFeeAfter));
        });

        it('Admin sets governance or collateralization wallets', async () => {
            pool.setGovernanceWallet(govWallet);
            pool.setCollateralizationWallet(colWallet);
            assert.equal(govWallet, (await pool.getGovernanceWallet()));
            assert.equal(colWallet, (await pool.getCollateralizationWallet()));
        });
        ...
        it('Admin finalizes pool', async () => {
            ...
            assert.equal(1, fromWei(adminBal));
        ...
        it('Cant setPublicSwap, set fees when finalized', async () => {
            await truffleAssert.reverts(pool.setPublicSwap(false), 'ERR_IS_FINALIZED');
            await truffleAssert.reverts(pool.setLiquidityProvidersFee(toWei('0.01')), 'ERR_IS_FINALIZED');
            await truffleAssert.reverts(pool.setGovernanceFee(toWei('0.01')), 'ERR_IS_FINALIZED');
            await truffleAssert.reverts(pool.setCollateralizationFee(toWei('0.01')), 'ERR_IS_FINALIZED');
        });
    ...
    describe('User interactions', () => {
        ...
        it('User1 joins pool', async () => {
            await pool.joinPool(toWei('0.05'), [MAX, MAX, MAX], { from: user1 });
        ...
        */
        ...
        it('Fails joins exits with limits', async () => {
            await truffleAssert.reverts(
                pool.joinPool(toWei('0.1'), [toWei('1'), toWei('1'), toWei('1')]),
                'ERR_LIMIT_IN',
            );

            await truffleAssert.reverts(
                pool.exitPool(toWei('0.1'), [toWei('10'), toWei('10'), toWei('10')]),
                'ERR_LIMIT_OUT',
            );

            await truffleAssert.reverts(
                pool.joinswapExternAmountIn(DAI, toWei('100'), toWei('0.01')),
                'ERR_LIMIT_OUT',
            );

            await truffleAssert.reverts(
                pool.joinswapPoolAmountOut(DAI, toWei('0.01'), toWei('100')),
                'ERR_LIMIT_IN',
            );

            await truffleAssert.reverts(
                pool.exitswapPoolAmountIn(DAI, toWei('0.01'), toWei('1000')),
                'ERR_LIMIT_OUT',
            );

            await truffleAssert.reverts(
                pool.exitswapExternAmountOut(DAI, toWei('1000'), toWei('0.01')),
                'ERR_LIMIT_IN',
            );
        });
    ...
    describe('BToken interactions', () => {
        it('Token descriptors', async () => {
            ...
            assert.equal(name, 'Polars Trading Pool Token');

            const symbol = await pool.symbol();
            assert.equal(symbol, 'PTPT');
        ...
        it('Token transfers', async () => {
            await truffleAssert.reverts(
                pool.transferFrom(user2, admin, toWei('0.1')),
                'ERR_BTOKEN_BAD_CALLER',
            );

            await pool.transferFrom(admin, user2, toWei('0.01'));
            await pool.approve(user2, toWei('0.1'));
            await pool.transferFrom(admin, user2, toWei('0.01'), { from: user2 });
        });
    });
});
